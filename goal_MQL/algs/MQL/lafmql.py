        ...
        for it in range(iterations):
           ...
           # critic_loss = (critic_loss_temp * beta_score).mean() yuan
            ...
            if it % self.policy_freq == 0:
                ...
                # actor_loss_temp = -1 * beta_score * lmbda * self.critic.Q1(obs, self.actor(obs, pre_act_rew), pre_act_rew).mean() + 0.1 * F.mse_loss(pi, action)
    ...
    def train_TD3(
        ...
        for it in range(iterations):
            ...
            if it % self.policy_freq == 0:
                ...
                # pi = self.actor(obs, pre_act_rew)
                #
                # actor_loss = -lmbda * self.critic.Q1(obs, self.actor(obs, pre_act_rew), pre_act_rew).mean() + F.mse_loss(pi, action)
                actor_loss_out += actor_loss.item()
