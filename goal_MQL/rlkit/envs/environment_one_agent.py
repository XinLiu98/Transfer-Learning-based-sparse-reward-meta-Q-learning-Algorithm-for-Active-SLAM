...
import sys
sys.path.append('/home/gzz/goal_MQL/rlkit/envs')
import agent
...
class GazeboDirEnv():
    def __init__(self, is_training=True, task={}, n_tasks=2, randomize_tasks=False):
        ...
        self.agent_1 = agent.Agent(num=1, threshold_goal=0.5, threshold_collision=0.13,
                                   flag_of_social=False)
    ...
    def step(self, action):
        ...
        t2 = time.time()
        reward= self.get_agent_reward(1)
        infos = dict(reward=reward,
                      task=self._task)

        return (observation, reward, done, arrive, infos, linear_speed, angular_speed)

    def sample_tasks(self, num_tasks):
        goal_model_position = Pose()
        mode = np.random.uniform(0., 1.0, size=(num_tasks,))
        # if mode < 1:
        #     temp_x = 3
        ...
        tasks = [{'target_1': target_1} for target_1 in mode]
        print('tasks', tasks)
        return tasks
    ...
    def reset_task(self, idx):
        ...
        self.mode = self._task['target_1']
        # self._goal_dir = self._task['target_1']
        # self._goal = self._goal_dir
        print('!!!!!!!!!_goal', self.mode)
        self.reset()
    ...
    def get_agent_reward(self, num):
        if num == 1:
            reward= self.agent_1._compute_reward()
        return reward
    ...
    def reset(self):
        ...
        self.robot_ModelState.pose.position.x =-2.5#(-2.5 * random.random()) - 0.5 # (-1.0 * random.random()) - 0.5 #todo 1--2
        self.robot_ModelState.pose.position.y = -2.5#(-2.5 * random.random()) + 0.5 #(-1.0 * random.random()) - 0.5
...
#        rospy.init_node(self.robot_ModelState)
        try:
            rospy.init_node("self.pub")
        ...
        try:
            ...
            mode = self.mode #random.random() * 4
            temp_x = 3#1#3#2#3
            temp_y = 2+2*mode#1 + 2 * mode#1 + 2 * mode#1 + 2 * mode#1 + 2 * mode#1 + 2 * mode#2 * mode#1 + 2 * mode#-2 - 2*mode #(6.0 * mode) - 3.0 #todo 1
            # if mode < 1:
            #     temp_x = 3
            #     temp_y = 0 + (6.0 * random.random()) - 3.0
            #     # print('!!!!!!!x',temp_x)
