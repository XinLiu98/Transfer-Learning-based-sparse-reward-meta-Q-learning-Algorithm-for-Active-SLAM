...
class Agent(object):
    def __init__(self, num, threshold_goal, threshold_collision, flag_of_social=False):
        ...
        rospy.Subscriber('/kobuki/laser/scan', LaserScan, self.callback_laser, queue_size=1)
    ...
    def get_state(self):
        ...
        if self.final_goal_distance < self.reached_goal_threshold: #threshold_goal=0.5
            self.done_reached_goal = True
            self.cumulated_steps_goal = 0
            #print(self.done_reached_goal)
       ...
       # observation_output = final_observation
        ...
        # for i in range(10):
        #     temp_image_x, temp_image_y = Transformation(lidar_10[i], x_10[i], y_10[i], yaw_10[i], x_10[-1], y_10[-1],
        #                                                 yaw_10[-1])
    ...
    def _compute_reward(self):
        ...
        if not (self.done_collision or self.done_reached_goal):
            ...
            # GOAL REWARD #*3
            reward_goal = 3 * 7.0 * (self.last_final_goal_distance - self.final_goal_distance)
            ...
            if self.reward_distance < 1: #0.5--1.5
                reward_collision = - 1.5 * (1 - (self.reward_distance - 0.25) / 0.75)
                if self.reward_distance < 0.5:
                    flag_ego_safety = 0
            ...
            if abs(self.odom.twist.twist.linear.x) < 0.3:
                reward_velocity = -0.1 * (0.3 - abs(self.odom.twist.twist.linear.x)) / 0.3
            
            if self.social == False:
                reward_social = 0

            reward = reward_goal + reward_collision + reward_velocity #+ reward_goal_distance #+ reward_rotation + reward_velocity
            #if self.num == 1:
                #print("reward_goal ", reward_goal)
                #print("reward_collision ", reward_collision)
            print("reward_goal ", reward_goal)
            print("reward_collision ", reward_collision)
            # print("reward_goal_distance ", reward_goal_distance)
            print("reward ", reward)
        ...
        if ((self.done_reached_goal) and (self.odom.twist.twist.linear.x != 0.0) and (self.odom.twist.twist.angular.z != 0.0)):
            # rospy.logwarn("TurtleBot2 reached GOAL !!! ==> Agent " + str(self.num))
            rospy.logerr("TurtleBot2 reached GOAL !!! ==> Agent " + str(self.num))
            reward = 20.0
