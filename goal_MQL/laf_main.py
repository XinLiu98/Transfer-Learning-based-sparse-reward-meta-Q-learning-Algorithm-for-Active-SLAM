...
parser.add_argument('--check_point_dir', default='./ck/4-d1/lafmql')
...
#TODO 1.1 1200-200
parser.add_argument('--max_path_length', type=int, default = 400)#todo 4
...
def evaluate_policy(eval_env,
    ...
    for tidx in etasks:
        ...
        for _ in range(eparams.num_evals):
            obs = eval_env.reset()
            i = 0
            # while obs.shape != torch.Size([1, 371]):
            #     obs = eval_env.reset()
            #     i = i + 1
            #     if i == 100:
            #         break
            ...
            while not (done or arrive) and step < eparams.max_path_length :
                ...
                print('main304',action)
                new_obs, reward, done, arrive, _,linear_speed, angular_speed = eval_env.step(action)
                avg_reward += reward
                step += 1
...
if __name__ == "__main__":
    ...
    loggerr = SummaryWriter(log_dir="data/444-d1/lafmql")
    #log_step_interval = 10  # 记录的步数间隔

    ##############################
    #### Init env, model, alg, batch genera2tor etc
    #### Step 1: build env
    #### Step 2: Build model
    ...
    if args.enable_context == True:
            ...
            if args.only_concat_context == 3: # means use LSTM with action_reward_state as an input
                input_dim_context = 2 + reward_dim + 371 #369----371
    ...
    actor_target_net = net.Actor(#action_space = [-1,1],
                                    hidden_sizes =args.hidden_sizes,
                                    input_dim = actor_idim,
                                    max_action = max_action,
                                    enable_context = args.enable_context,
    ...
    critic_net = net.Critic(#action_space =[-1,1],
                                hidden_sizes =args.hidden_sizes,
                                input_dim = (1,371),
                                enable_context = args.enable_context,
    ...
    critic_target_net = net.Critic(#action_space = [-1,1],
                                        hidden_sizes =args.hidden_sizes,
                                        input_dim = (1,371),
                                        enable_context = args.enable_context,
                                        dim_others = dim_others,
    ...
    if str.lower(args.alg_name) == 'mql':
        ...
        # import algs.MQL.lafmqlbf as alg
        import algs.MQL.lafmql as alg
    ...
    print("Observation_space:", [1,371] )
    print("Action space:", [1,2] )
    ...
    while update_iter < args.total_timesteps:
        ...
        for tidx in train_indices:
            ...
            if ((episode_num % args.log_interval == 0 or episode_num % len(train_tasks)/2 == 0) or episode_num == 1 ):
                ...
                loggerr.add_scalar('eprewmean', float(safemean([epinfo['r'] for epinfo in epinfobuf])), global_step=episode_num)
                loggerr.add_scalar('eplenmean', float(safemean([epinfo['l'] for epinfo in epinfobuf])), global_step=episode_num)
                ...
                loggerr.add_scalar('angular_speed', float(safemean([epinfo['z'] for epinfo in epinfobuf])),
                                   global_step=episode_num)
                # loggerr.add_scalar('reward_goal', float(safemean([epinfo['g'] for epinfo in epinfobuf])),
                #                    global_step=episode_num)
                # loggerr.add_scalar('reward_collision', float(safemean([epinfo['c'] for epinfo in epinfobuf])),
                #                    global_step=episode_num)
                ...
                if 'actor_mmd_loss' in alg_stats:
                    ...
                    loggerr.add_scalar('critic_mmd_loss', float(alg_stats['critic_mmd_loss']), global_step=episode_num)
                    loggerr.add_scalar('actor_mmd_loss', float(alg_stats['actor_mmd_loss']), global_step=episode_num)
                if 'beta_score' in alg_stats:
                     logger.record_tabular("beta_score", float(alg_stats['beta_score']))
                     loggerr.add_scalar('beta_score', float(alg_stats['beta_score']), global_step=episode_num)
